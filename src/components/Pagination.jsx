...
const Pagination = ({ queries }) => {
    ...
    const [pg, setPg] = useState(pagination[0].startIndex);
    const navigate = useNavigate();

    useEffect(() => {
        setPg(pagination[0].startIndex);
    }, [query]);
        ...
        <div className="flex flex-col items-center py-14 max-w-[700px]">
            <div className="relative text-blue-500">
                {queries.previousPage && (
                    ...
                    >
                        <FiChevronLeft size={25} className="cursor-pointer" />
            ...
            <div className="flex gap-3 text-blue-300 text-sm">
                {pagination.map((el) => (
                    <span
                        key={el.page}
                        onClick={() => paginationHandler(el.startIndex)}
                        className={`cursor-pointer ${
                            pg === el.startIndex ? "text-black" : ""
                        }`}
                    >
                        {el.page}
                    </span>
